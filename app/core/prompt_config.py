...
def detect_response_language(user_question: str, supported: set[str], fallback: str = "en") -> str:
    ...
    patterns = {
        ...
        "fr": [" le ", " la ", " les ", " des ", " une ", " et ", " pas ", " vous ", " avec ", " dans ", " pour ", " est ", " comment ", " un "],
        "es": [" el ", " la ", " los ", " las ", " de ", " que ", " y ", " para ", " con ", " en ", " un ", " una ", " cómo ", " como "],
        "pt": [" o ", " a ", " os ", " as ", " de ", " que ", " e ", " para ", " com ", " não ", " nao ", " em ", " um ", " uma ", " como ", " você ", " voce ", " por que ", " porque ", " criar ", " aplicativo ", " aplicação ", " aplicacao ", " posso ", " meu ", " minha ", " botão ", " botao "],
    ...
    for lang_code, lang_patterns in patterns.items():
        ...
        # Special boost for Spanish: "¿" is a strong marker
        if lang_code == "es" and "¿" in user_question:
            count += 1
        # Special boost for Portuguese: "não/nao" and "você/voce" are strong markers
        if lang_code == "pt":
            if " não " in normalized or " nao " in normalized:
                count += 2
            if " você " in normalized or " voce " in normalized:
                count += 2
            # Boost for Portuguese diacritics: ãõçáéíóúâêôà
            if re.search(r'[ãõçáéíóúâêôà]', user_question):
                count += 2
        if count > 0:
