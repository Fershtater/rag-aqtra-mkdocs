...
class ConversationService:
    ...
    def __init__(self, db_sessionmaker: Optional[async_sessionmaker[AsyncSession]] = None, sessionmaker: Optional[async_sessionmaker[AsyncSession]] = None):
        """
        Initialize conversation service.
        
        Args:
            db_sessionmaker: Database sessionmaker (can be None if DB not configured)
            sessionmaker: Alternative parameter name for backward compatibility (used if db_sessionmaker is None)
        """
        # Support both parameter names for backward compatibility
        self.db_sessionmaker = db_sessionmaker if db_sessionmaker is not None else sessionmaker
