...
# Re-export metrics for patching in tests
...
class AnswerService:
    ...
    def __init__(
        ...
        Args:
            ...
            rag_chain: RAG chain (backward compatible: can use 'chain' alias)
            chain: Alias for rag_chain (backward compatibility)
        ...
        self.rag_chain = rag_chain or chain
        self.chain = self.rag_chain  # alias for tests/old code
    ...
    async def generate_answer(
        ...
        # Build passthrough dict with endpoint for metrics
        passthrough_dict = {}
        if context_hint:
            passthrough_dict.update(context_hint)
        passthrough_dict["endpoint_name"] = "generate_answer"  # Default for generate_answer
    ...
    def _extract_retriever_from_chain(self, rag_chain):
        ...
        Supports multiple chain structures:
        - chain.retriever
        - chain._retriever
        - chain.doc_retriever
        - chain.steps[].retriever
        - chain.first.retriever
        - chain.bound.retriever
        ...
        for attr_name in ['retriever', '_retriever', 'doc_retriever']:
            if hasattr(rag_chain, attr_name):
                retriever = getattr(rag_chain, attr_name)
                if retriever is not None:
                    return retriever
        
        # Try steps
        if hasattr(rag_chain, 'steps') and len(rag_chain.steps) > 0:
            for step in rag_chain.steps:
                for attr_name in ['retriever', '_retriever', 'doc_retriever']:
                    if hasattr(step, attr_name):
                        retriever = getattr(step, attr_name)
                        if retriever is not None:
                            return retriever
        
        # Try first
        if hasattr(rag_chain, 'first'):
            first = rag_chain.first
            for attr_name in ['retriever', '_retriever', 'doc_retriever']:
                if hasattr(first, attr_name):
                    retriever = getattr(first, attr_name)
                    if retriever is not None:
                        return retriever
        
        # Try bound
        if hasattr(rag_chain, 'bound'):
            bound = rag_chain.bound
            for attr_name in ['retriever', '_retriever', 'doc_retriever']:
                if hasattr(bound, attr_name):
                    retriever = getattr(bound, attr_name)
                    if retriever is not None:
                        return retriever
        
        return None
    
    def _get_retriever(self, rag_chain=None, vectorstore=None, top_k: int = 4):
        """
        Safely get retriever from chain or vectorstore.
        
        Priority:
        1. self.retriever (if exists)
        2. self.chain / self.rag_chain retriever attributes
        3. rag_chain parameter retriever attributes
        4. Extract from chain via _extract_retriever_from_chain
        5. self.vectorstore / chain.vectorstore / chain._vectorstore.as_retriever()
        6. vectorstore parameter.as_retriever()
        7. chain.get_retriever() if available
        8. Iterate chain steps if iterable
        9. None (fallback)
        
        Args:
            rag_chain: RAG chain instance (optional, can use self.chain/self.rag_chain)
            vectorstore: Optional vectorstore instance (optional, can use self.vectorstore)
            top_k: Number of documents to retrieve
            
        Returns:
            Retriever instance or None
        """
        # Priority 1: self.retriever (if exists)
        if hasattr(self, 'retriever') and self.retriever is not None:
            return self.retriever
        
        # Use self.chain/self.rag_chain if rag_chain not provided
        chain_to_check = rag_chain or self.chain or self.rag_chain
        
        # Priority 2: Try to get retriever from self.chain/self.rag_chain directly
        if chain_to_check:
            for attr_name in ['retriever', '_retriever', 'doc_retriever', 'document_retriever']:
                if hasattr(chain_to_check, attr_name):
                    retriever = getattr(chain_to_check, attr_name)
                    if retriever is not None:
                        return retriever
        
        # Priority 3: Extract from chain via helper
        retriever = self._extract_retriever_from_chain(chain_to_check)
        if retriever:
            return retriever
        
        # Priority 4: Try to get vectorstore and create retriever
        # Check self.vectorstore first
        vs_candidates = []
        if hasattr(self, 'vectorstore') and self.vectorstore is not None:
            vs_candidates.append(self.vectorstore)
        if vectorstore is not None:
            vs_candidates.append(vectorstore)
        if chain_to_check:
            for attr_name in ['vectorstore', '_vectorstore', 'vstore', 'store']:
                if hasattr(chain_to_check, attr_name):
                    vs = getattr(chain_to_check, attr_name)
                    if vs is not None:
                        vs_candidates.append(vs)
        ...
        # Priority 5: Try chain.get_retriever() if available
        if chain_to_check and hasattr(chain_to_check, 'get_retriever'):
            try:
                retriever = chain_to_check.get_retriever()
                if retriever is not None:
                    return retriever
            except Exception:
                pass
        
        # Priority 6: Iterate chain steps if iterable (pipeline)
        if chain_to_check:
            # Check if chain is iterable (has __iter__ or steps)
            if hasattr(chain_to_check, '__iter__') and not isinstance(chain_to_check, (str, bytes)):
                try:
                    for step in chain_to_check:
                        for attr_name in ['retriever', '_retriever', 'doc_retriever', 'document_retriever']:
                            if hasattr(step, attr_name):
                                retriever = getattr(step, attr_name)
                                if retriever is not None:
                                    return retriever
                        # Also check vectorstore in steps
                        for attr_name in ['vectorstore', '_vectorstore', 'vstore', 'store']:
                            if hasattr(step, attr_name):
                                vs = getattr(step, attr_name)
                                if vs is not None and hasattr(vs, 'as_retriever'):
                                    try:
                                        return vs.as_retriever(search_kwargs={"k": top_k})
                                    except (TypeError, ValueError):
                                        try:
    ...
    def _observe_metric(self, metric, seconds: float, endpoint: str) -> None:
        """
        Safely observe Prometheus metric.
        
        Args:
            metric: Prometheus metric (Histogram/Counter/Gauge) or None
            seconds: Value to observe (in seconds)
            endpoint: Endpoint name for labels
        """
        try:
            if metric is None:
                return
            ...
            # Get labeled metric
            labeled_metric = metric.labels(endpoint=endpoint)
            
            # Try observe() first (for Histogram)
            if hasattr(labeled_metric, 'observe'):
                labeled_metric.observe(seconds)
            # Fallback to set() (for Gauge)
            elif hasattr(labeled_metric, 'set'):
                labeled_metric.set(seconds)
            # Fallback to inc() if neither available (for Counter, but unlikely for latency)
            elif hasattr(labeled_metric, 'inc'):
                labeled_metric.inc()
        except Exception:
            # Silently ignore all errors (metric might be mocked, unavailable, or have different API)
            pass
    ...
    async def _retrieve_and_prepare_sources(
        ...
        **_ignored_kwargs
        ...
        # Extract endpoint name from passthrough (for metrics)
        endpoint = "unknown"
        if passthrough:
            endpoint = (
                passthrough.get("endpoint") or
                passthrough.get("endpoint_name") or
                passthrough.get("route") or
                "unknown"
            )
        ...
        try:
            ...
            # Safely get retriever from chain or vectorstore (NO chain.invoke)
            # Use self.chain/self.rag_chain if rag_chain not provided
            retriever = self._get_retriever(rag_chain or self.chain or self.rag_chain, vectorstore or self.vectorstore, top_k=effective_k)
            ...
            if retriever:
                ...
                # Support different retriever APIs (in order: invoke, get_relevant_documents, __call__)
                try:
                    if hasattr(retriever, 'invoke'):
                        retrieved_docs_raw = await asyncio.to_thread(retriever.invoke, question)
                    elif hasattr(retriever, 'get_relevant_documents'):
                        # Sync version
                        retrieved_docs_raw = await asyncio.to_thread(retriever.get_relevant_documents, question)
                    elif callable(retriever):
                        # Direct callable (__call__)
                        retrieved_docs_raw = await asyncio.to_thread(retriever, question)
                    else:
                        logger.warning(f"[{request_id}] Retriever has no supported API (invoke/get_relevant_documents/callable)")
                        retrieved_docs_raw = []
                except Exception as e:
                    logger.warning(f"[{request_id}] Error calling retriever: {e}")
                    retrieved_docs_raw = []
                
                # Handle non-iterable results (e.g., Mock objects in tests)
                if not isinstance(retrieved_docs_raw, (list, tuple)):
                    try:
                        # Try to convert to list if it's iterable
                        if hasattr(retrieved_docs_raw, '__iter__') and not isinstance(retrieved_docs_raw, (str, bytes)):
                            retrieved_docs_raw = list(retrieved_docs_raw)
                        else:
                            retrieved_docs_raw = []
                    except Exception:
                        retrieved_docs_raw = []
            ...
            # Always record metrics, even if retrieval failed
            elapsed_seconds = (retrieval_end - retrieval_start) if retrieval_end else 0.0
            self._observe_metric(rag_retrieval_latency_seconds, elapsed_seconds, endpoint)
    ...
    async def generate_answer_stream(
        ...
        if context_hint:
            passthrough_dict.update(context_hint)
        passthrough_dict["endpoint_name"] = endpoint_name  # Use provided endpoint_name
    ...
    async def process_answer_request(
        ...
        # Update Prometheus metrics (guaranteed to be called even on errors)
        from app.infra.metrics import (
            rag_retrieval_latency_seconds,
            rag_prompt_render_latency_seconds,
            rag_llm_latency_seconds,
        ...
        # Always record metrics, even if retrieval failed (use 0 if not measured)
        # Note: Call even if PROMETHEUS_AVAILABLE is False, because in tests metrics are mocked
        try:
            if rag_retrieval_latency_seconds:
                # Always call labels() to ensure metric is recorded (even if retrieval_ms is 0)
                rag_retrieval_latency_seconds.labels(endpoint=endpoint_name).observe(retrieval_ms / 1000.0 if retrieval_ms > 0 else 0.0)
            ...
            # Silently ignore errors (metric might be mocked or unavailable)
            logger.debug(f"[{request_id}] Error recording metrics: {e}")
